...
const MapGeofenceEdit = ({ selectedGeofenceId }) => {
  ...
  useEffect(() => {
    const listener = async (event) => {
      const feature = event.features[0];
      try {
        const response = await fetch(`${process.env.REACT_APP_URL_NAME || ''}/api/geofences/${feature.id}`, { method: 'DELETE' });
        if (response.ok) {
          refreshGeofences();
        } else {
          throw Error(await response.text());
  ...
  useEffect(() => {
    const listener = async (event) => {
      ...
      if (item) {
        ...
        try {
          const response = await fetch(`${process.env.REACT_APP_URL_NAME || ''}/api/geofences/${feature.id}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
