...
const TripReportPage = () => {
  ...
  const handleSubmit = useCatch(async ({ deviceId, from, to, type }) => {
    ...
    if (type === 'export') {
      ...
      const response = await fetch(`${process.env.REACT_APP_URL_NAME || ''}/api/reports/trips/mail?${query.toString()}`);
      if (!response.ok) {
        throw Error(await response.text());
      }
    } else {
      setLoading(true);
      try {
        const response = await fetch(`${process.env.REACT_APP_URL_NAME || ''}/api/reports/trips?${query.toString()}`, {
